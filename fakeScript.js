...
(async function () {
    ...
    function runScript(config, coordinates) {
        const { sendMode, unitsAndAmounts, whatSend, excludedUnits } = config;
        ...
        // fill unit fields
        getWhatUnitsToSend(whatSend, unitAmountsTransformed, excludedUnits);
    }

    // Helper: What units to send
    function getWhatUnitsToSend(whatSend, unitsAndAmounts, excludedUnits = []) {
        let totalTroops = 0;
        ...
        switch (whatSend) {
            ...
            case 'catapult_then_ram':
                ...
                break;
            case 'fake_limit':
                fillUnitAmountsFakeLimitWorld(excludedUnits);
                break;
            default:
    ...
    function fillUnitAmountsFakeLimitWorld(excludedUnits = []) {
        ...
        if (cats > 0 && !excludedUnits.includes('catapult')) {
            document.forms[0].catapult.value = 1;
            fakePop = fakePop - 8;
        } else {
            if (rams > 0 && !excludedUnits.includes('ram')) {
